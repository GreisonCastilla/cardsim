...
interface HyperspatialWidgetProps {
  pid: PlayerId;
...
export function HyperspatialWidget({
  pid,
  zones,
  cards,
  handleCardHover,
  handleContextMenu,
}: HyperspatialWidgetProps) {
  const hsKey = `${pid}_hyperspatial` as ZoneName;
  const cardIds = zones[hsKey] || [];
  ...
  return (
    <div className="relative flex flex-col items-center select-none">
      ...
      <motion.div
        ...
        whileHover={{ scale: 1.05, filter: "brightness(1.1) drop-shadow(0 0 15px rgba(147,51,234,0.5))" }}
        whileTap={{ scale: 0.95 }}
        className={cn(
          ...
          (isOpen || isOver) && "drop-shadow-[0_0_15px_rgba(147,51,234,0.6)] border-purple-500/50",
          isOver && "bg-purple-500/20 ring-2 ring-purple-500 ring-offset-2 ring-offset-transparent"
        )}
        onClick={(e) => {
          e.stopPropagation();
          setIsOpen(!isOpen);
      ...
      >
        {topCardId ? (
          <div className="w-full h-full relative">
            ...
            />
            {totalCards > 1 && (
              <div className="absolute top-1 right-1 bg-purple-600 text-white text-[7px] font-black px-1 rounded-sm shadow-lg border border-purple-400 z-10">
                {totalCards}
              </div>
            )}
          </div>
        ...
        {/* Drop Indicator */}
        {isOver && (
          <div className="absolute inset-0 bg-purple-500/20 mix-blend-screen" />
        )}
      </motion.div>
              ...
              >
                ...
                <motion.div
                  ...
                  className={cn(
                    ...
                    isOver ? "border-purple-500 shadow-[0_0_50px_rgba(168,85,247,0.5)] scale-105" : "border-purple-900 shadow-[0_0_40px_rgba(168,85,247,0.3)]",
                      ...
                      cardIds.map((id) => {
                        ...
                        return (
                          <motion.div
                            ...
                            onContextMenu={(e) => handleContextMenu(e, card, hsKey)}
                          >
                            <div className="w-[48px] h-[64px] relative flex items-center justify-center">
                              <Card
                                ...
                                zone={hsKey}
                                isStatic
                                onHover={(c) => handleCardHover(c, hsKey)}
                ...
                <motion.div
                  ...
                  className={cn(
                    ...
                    isOver ? "border-t-purple-500" : "border-t-purple-900"
                  )}
